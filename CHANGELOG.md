...
- **Agent effort tiers tuned for Claude 4.8 prompting best practices.** `op` (foundation/cross-cutting) → `xhigh` (the documented sweet-spot for heavy coding/agentic work), `opm` (multi-file refactor) and `orc` (orchestrator) → `high` (clears the under-thinking risk 4.8 has at `medium` on intelligence-sensitive work). `xhigh` requires a Claude Code build whose `--effort` accepts it (run `claude --help` to confirm; revert the agent to `high` if not). The `CLAUDE.md` subagent guidance gained an effort ladder, an adaptive-thinking note (thinking is self-gated — tune `effort`, don't toggle thinking), and a finding-vs-verification split for review agents (don't tell 4.8 to "be conservative" at the finding stage — it drops real bugs).
