...
- **`claudii skills-cost` rejects unknown flags** with an actionable error instead of silently ignoring them (`--foo` was a no-op; a typo like `--day 7` silently billed 30 days). The dispatcher now passes only the actual flags to the parser.
