...
### Fixed
- **`claudii cost --json`/`--tsv` and `claudii trends --json` are now valid under a comma-decimal locale.** The aggregators print costs via awk `printf %.4f`/`%.2f`, which honors `LC_NUMERIC` — under e.g. `LC_ALL=de_DE.UTF-8` they emitted `"cost":12,0000` / `"cost":0,00`, breaking `jq` and the TSV number column (a documented `--json`/`--tsv` contract producing unparseable output). The json/tsv formatting awk now runs under `LC_ALL=C` (its output is pure ASCII); the pretty branch stays in the user locale because it prints UTF-8 box chars (which `LC_ALL=C` would mangle) and is already locale-immune (`$` via `fmt_usd`). Same fix shipped in `cost --forecast`. Verified byte-identical C/en_US output (no regression) and `jq`-valid de_DE output; guarded de_DE regression asserts added. (Note: `skills-cost`'s outlier-flag math has the same root cause — awk parsing dot-decimals under a comma locale truncates at the dot — tracked separately as it spans the internal median/threshold chain, not just output.)
