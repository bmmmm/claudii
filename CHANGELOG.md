...
### Changed
- **Shared `lib/fmt.awk` + `lib/render.sh` renderers** — the token/large-number formatter `fmt_tok` was duplicated inline in `lib/cmd/cost.sh` and `lib/trends.awk` (awk) and mirrored as `_insights_short_num` in `lib/cmd/insights.sh` (a per-call `awk` fork). The awk copies now live in `lib/fmt.awk` (`fmt_tok`/`rep`/`bar`/`bar_filled`, loaded via `-f` in trends and string-injection in cost); the bash side moved to `lib/render.sh` as pure-bash `_fmt_tok` (no fork) plus `_cache_hit_pct`/`_bar`/`_bar_filled`. Foundation for the token-usage display shift (token throughput as the primary metric across all views, `$` demoted to `claudii cost`); verified byte-identical cost/trends/cache output, green under `/bin/bash` 3.2.
