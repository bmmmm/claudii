...
# Must actually find + emit the release notes, not the "No changelog entry"
# fallback. Assert a markdown subsection heading ("### Added/Changed/Fixed") is
# present — the awk extraction emits those verbatim, the fallback emits none.
# We can NOT grep for the literal "No changelog entry" string: a release whose
# own notes document the changelog command's fallback (v0.20.0 does exactly that)
# contains that phrase legitimately in the resolved notes, so a substring check
# false-positives. The "###" marker only appears when real notes were resolved.
assert_contains "changelog: resolves the release notes (not the empty fallback)" \
  "###" "$changelog_out"
