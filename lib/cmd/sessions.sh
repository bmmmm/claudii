...
# ── _cmd_skills_cost — per-skill (or per-plugin) cost breakdown ───────────────
# Usage: claudii skills-cost [--days N] [--plugins] [--json]
# Reads attribution_skills (or attribution_plugins) by aggregating the insights
# cache files directly — the claudii-insights merge command gains these fields
# when Agent A merges; until then we aggregate them here with the same cutoff logic.
# Token pricing uses the same per-model rates as the cost command.
_cmd_skills_cost() {
  _cfg_init
